...
| 角色 | 能力 |
|------|------|
| `SUPER_ADMIN` | 所有后台治理接口 + 用户管理 + 角色变更 |
| `ADMIN` | 后台治理接口（审计日志查看、用户列表等），不含角色变更 |
| `USER` | 创建族谱（自动获得 `OWNER`）、访问自己有资源授权的族谱 |

- `USER` 创建族谱数量上限本阶段不限制，后续可按需增加配额机制。
- `USER` 之间默认互相隔离，只能通过协作者授权或分享链接访问他人族谱。
...
- 明文 token 长度：32 字节随机值，编码为 URL-safe Base64（43 字符）。
- 存储：对明文 token 做 SHA-256 hash 后存入 `token_hash` 字段，数据库中不保留明文。
- 验证流程：收到请求后对传入 token 计算 SHA-256，再与 `token_hash` 比对。

#### 分享链接频率限制

- 单份 publication 最多同时存在 20 条 `ACTIVE` 状态的分享链接，超出时拒绝创建并返回 `400`。
- 此限制防止恶意 `OWNER` 批量生成链接用于枚举攻击。
...
改造前：list 只返回”我创建的族谱”。
改造后：list 返回”我有资源授权的所有族谱”，包含我创建的 + 别人授权给我的。

建议响应中增加 `accessRole` 字段，标识当前用户在每份族谱中的角色（`OWNER`/`EDITOR`/`VIEWER`），以便前端区分展示”我的”和”共享给我的”。

建议支持按角色筛选的查询参数：`?role=OWNER`（仅我创建的）、`?role=EDITOR`（我可编辑的）等。
...
- `hideLivingSensitive`：隐藏在世成员的敏感字段（如详细地址、联系方式）。
- `hideContactInfo`：隐藏所有联系方式字段。
- `photoProxy`：照片通过分享链路代理访问，不暴露原始 `/api/photos/{id}` 路径。
- `maxExportDepth`：脱敏导出时的最大世代深度，防止通过完整导出绕过脱敏。

后续可扩展更多配置项，但初版应先固化这一套默认值。
...
- 对 `UserSubject`：如果该用户在 `publication_access` 中没有目标 publication 的任何记录，一律返回 `404`（而非 `403`），防止通过响应差异枚举他人资源。
- 对 `ShareSubject`：如果分享链接不属于目标 publication，返回 `404`。
- 仅当用户已被识别为协作者（有 access 记录）但动作权限不足时，才返回 `403`。示例：`VIEWER` 试图编辑 → `403`。
...
- 初版审计日志保留周期：90 天。
- 超过保留期的日志由 `@Scheduled` 定时任务物理删除。
- 保留周期可通过 `application.properties` 配置（`audit.retention-days=90`）。
- 后续如需合规要求，可升级为归档而非删除。
...
当用户在编辑过程中被降级或撤销权限（如 `EDITOR` → `VIEWER`），后端会在下一次写操作时返回 `403`。前端需处理以下场景：

- 编辑保存失败（`403`）：提示"您的编辑权限已被变更，当前仅可查看"，自动切换为只读模式。
- 页面刷新时重新拉取 `accessRole`，确保 UI 状态与后端一致。
- 不需要 WebSocket/SSE 实时推送——被动发现 + 用户提示已足够，本阶段不引入推送机制。

### list 页面展示

- 区分"我创建的"（`role=OWNER`）和"共享给我的"（`role=EDITOR`/`VIEWER`）两种视图。
- 响应中的 `accessRole` 字段驱动前端展示不同的操作按钮（编辑/只读）。

### 错误页面

- `403`：展示"权限不足"页面，引导用户联系族谱所有者。
- `410`：展示"分享链接已失效"页面。
- `404`：统一展示"资源不存在"，不泄露是否因权限原因返回。
...
1. 建立统一授权抽象
2. 修复 publication 主链路对象级权限（含 `list` 语义改造 + `accessRole` 返回）
3. 收口照片、导出、历史等旁路
4. 引入分享链路与脱敏投影
5. 增加协作者与分享管理 UI（前端含 403/410 错误页面、权限变更提示、角色筛选）

## 验收标准

当以下条件全部满足时，本设计视为落地成功：

1. 站内用户不能通过修改 id 越权访问他人族谱。无 access 记录的请求一律返回 `404`，不可通过响应差异枚举资源。
...
7. `list` 接口正确返回"我创建的 + 别人授权给我的"全部族谱，且每条记录携带 `accessRole` 字段。
8. 单份 publication 的 `ACTIVE` 分享链接数量不超过 20 条。
