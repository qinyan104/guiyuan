...
const FAMILY_BRANCH_MODES = new Set<FamilyBranchMode>(['married-out', 'uxorilocal'])
...
function validatePerson(personId: string, value: unknown): ValidationIssue[] {
  ...
  if (!isOptionalBoolean(person.deceased)) {
    issues.push(issue('invalid-person', `people.${personId}.deceased`, `人物 ${personId} 的 deceased 必须是布尔值。`))
  }
...
export function normalizeSettings(settings: PublicationSettings): PublicationSettings {
  return {
    ...
    cardShadowOpacity: clampNumber(settings.cardShadowOpacity, 0, 40),
