...
function chineseYear(value: string): string {
  return value.split("").map((digit) => CHINESE_YEAR_DIGITS[Number(digit)] || digit).join("")
}

function genealogyDate(value: string): string {
  const clean = markdownText(value)
  const dateParts = clean.match(/^(\d{4})[-/.](\d{1,2})[-/.](\d{1,2})$/)
  if (dateParts) return `${chineseYear(dateParts[1])}年${chineseNumber(Number(dateParts[2]))}月${chineseNumber(Number(dateParts[3]))}日`
  return clean
    .replace(/\d{4}(?=年)/g, chineseYear)
    .replace(/\d+(?=[月日])/g, (part) => chineseNumber(Number(part)))
}
...
function familyChildrenFor(personId: string, data: PublicationData, childPersonIds: ReadonlySet<string>): ChildReference[] {
    ...
    family.adults.includes(personId)
      ? family.children
          ...
          .map((childId) => ({
            ...
            mainLine: isMainLineChild(family.adults, personId, resolveFamilyBranchMode(data, family.id, childPersonIds)),
          }))
      : [],
  )
...
function childIdsFor(personId: string, data: PublicationData, childPersonIds: ReadonlySet<string>): string[] {
  return familyChildrenFor(personId, data, childPersonIds).filter((reference) => reference.mainLine).map((reference) => reference.personId)
}
...
function buildLineage(data: PublicationData, childPersonIds: ReadonlySet<string>): LineageNode[] {
  ...
  const queue = rootIds(data, childPersonIds).map((personId) => ({ personId, generation: generationFor(personId, data, childPersonIds) }))
  let cursor = 0
  ...
  while (cursor < queue.length) {
    ...
    result.push(next)
    queue.push(...childIdsFor(next.personId, data, childPersonIds).map((personId) => ({ personId, generation: next.generation + 1 })))
  }
...
function spouseNames(personId: string, data: PublicationData): string[] {
  return [...new Set(
    Object.values(data.families)
      ...
      .flatMap((family) => family.adults.filter((id) => id !== personId && data.people[id]).map((id) => spouseName(data.people[id]))),
  )]
}
...
function childClause(personId: string, data: PublicationData, childPersonIds: ReadonlySet<string>): string | null {
  ...
  const label = genders.size === 1 && genders.has("male") ? "子" : genders.size === 1 && genders.has("female") ? "女" : "子女"
  const names = children.map((reference, index) => {
    const branch = reference.branchMode === "married-out" ? "（外嫁支）" : reference.branchMode === "uxorilocal" ? "（招婿支）" : ""
...
function personMarkdown(node: LineageNode | { personId: string }, data: PublicationData, childPersonIds: ReadonlySet<string>): string {
...
export function buildPublicationMarkdown(data: PublicationData): string {
  ...
  const coveredIds = new Set(
    lineage.flatMap((node) => [node.personId, ...Object.values(data.families).filter((family) => family.adults.includes(node.personId)).flatMap((family) => family.adults)]),
  )
  const remaining = Object.keys(data.people).filter((personId) => !coveredIds.has(personId)).map((personId) => ({ personId }))
