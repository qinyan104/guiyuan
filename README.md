...
归源是一套面向家族谱牒整理、协作修订与出版数据提取的数字化工具，覆盖 Web 端无限画布族谱工作台、Markdown 出版数据导出、宗族洞察大屏、家族全景编年史以及 Docker 一键容器化部署方案。
...
- **族谱元数据提取**：从当前出版数据读取标题、堂号、源流、世代、人物和亲属关系。
- **简单 Markdown 排版**：按世代和人物关系输出标题、摘要、基本信息和备注，便于继续在成熟工具中编辑。
- **直接下载文件**：在出版数据页面预览并下载 `.md` 文件，不保存额外书稿或排版状态。
