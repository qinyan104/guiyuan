...
归源是一套面向家族谱牒整理、协作修订与数据提取的数字化工作台，帮助用户把零散资料整理成可信的家族世系，并导出符合谱牒写法的 Markdown 文本。系统覆盖无限画布、关系校验、协作审核、隐私控制、数据交换和 Docker 部署。

## 产品边界

归源负责族谱数据的录入、整理、校验、协作和导出；当前内置轻量“族谱出版数据”页面，用于从结构化世系生成可校订的 Markdown 草稿。专业古籍的字体、页式、印刷和装帧排版仍建议交给 Typora、Obsidian、Word、WPS 或其他成熟工具完成，Markdown 是两者之间的简单数据交接格式。
...
- **族谱元数据提取**：读取标题、堂号、原籍、家训、世代、人物和亲属关系。
- **谱牒体例 Markdown**：按“一世、二世”、某公、配偶、生子、长子等传统表达组织正文。
- **轻量出版数据页**：通过 `/book-editor/publication/:publicationId` 预览生成结果，并下载 `.md` 文件，便于在成熟工具中继续校订和排版。

### 3. 东方传统 5 大雅致主题体系

全站各界面（工作台、数据导出、分析页、详情页、登录页等）支持一键切换并自动持久化：
...
- **谱牒 Markdown 导出**：将结构化族谱整理为可编辑、可移交的谱牒正文。
- **自包含独立 HTML 导出**：可导出单个完全不依赖外部服务器、脱机双击即开的交互式族谱网页，支持设置密码保护。
- **图形导出**：支持导出用于分享和存档的 SVG、PNG 与网页文件。
...
| 模块 | 核心技术 |
| :--- | :--- |
| **前端** | Vue 3, Vite 6, TypeScript, Pinia, Vue Router, Canvas 2D |
...
│   ├── src/components/         # 基础组件、工作台等 UI 组件
│   ├── src/composables/        # 组合式业务逻辑与状态函数
│   ├── src/features/           # 出版数据、导出分享、历史追溯、版本冲突、智能校验
│   ├── src/lib/                # 世系树布局算法、亲属计算器
